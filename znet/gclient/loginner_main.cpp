...
{        
	...
	if (argc!=9  || access(argv[1], R_OK) == -1)
	{
...
// ÐÞ¸ÄÅäÖÃÎÄ¼þ
	conf->put("GLinkClient", "address", argv[6]);
	conf->put("GLinkClient", "port", argv[7]);

	printf("new address %s,new port %s\n", conf->find("GLinkClient", "address").c_str(), conf->find("GLinkClient", "port").c_str());
// end
...
//		manager->SetRoomID(atoi(argv[6]));
		...
		Thread::HouseKeeper::AddTimerTask(new PrintClientStateTask(manager),20);
	}
