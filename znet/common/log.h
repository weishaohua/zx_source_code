...
namespace GNET
{
	...
	public:
		static const char * prioritystring( int __priority )
		{
			...
			static char * sz_money   = "money_change";
                                                static char * sz_bot   = "bot";
	...
	public:
		...
		//日志规范化修改
		static void login( Octets account, int userid, unsigned int sid, std::string peer,Octets mid)
		{
			char buf[128];
                                                char temp[256];
			int len = std::min(account.size(),sizeof(buf)-1);
                                                int midlen = std::min(mid.size(),sizeof(temp)-1);
			memcpy( buf, (char*)account.begin(), len );
                                                memcpy( temp, (char*)mid.begin(), midlen );
			buf[len] = 0;
                                                buf[midlen] = 0;
			logvital( LOG_NOTICE, "formatlog:login:account=%s:userid=%d:sid=%u:peer=%s:mid=%s",
							buf, userid, sid, peer.c_str() ,temp);
		}
