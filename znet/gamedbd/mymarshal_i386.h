...
public:
	...
	public:
		mutable int depth;
		OctetsStream() : pos(0) {depth=1;}
		OctetsStream(const Octets &o) : data(o), pos(0) {depth=1;}
		OctetsStream(const OctetsStream &os) : data(os.data), pos(0) {depth=1;}
		...
		virtual OctetsStream& operator << (char x)               { return push_byte(x); }
		virtual OctetsStream& operator << (unsigned char x)      { return push_byte(x); }
		virtual OctetsStream& operator << (bool x)               { return push_byte(x); }
		virtual OctetsStream& operator << (short x)              { return push_byte(byteorder_16(x)); }
		virtual OctetsStream& operator << (unsigned short x)     { return push_byte(byteorder_16(x)); }
		virtual OctetsStream& operator << (int x)                { return push_byte(byteorder_32(x)); }
		virtual OctetsStream& operator << (unsigned int x)       { return push_byte(byteorder_32(x)); }
		...
		*/
		virtual OctetsStream& operator << (int64_t x)            { return push_byte(byteorder_64(x)); }
		...
		virtual OctetsStream& operator << (const Marshal &x)     { return x.marshal(*this); }
		virtual OctetsStream& operator << (const Octets &x)  
		{
		...
		virtual OctetsStream& push_byte(const char *x, size_t len)
		{
			data.insert(data.end(), x, len);
			return *this;
		}
		virtual OctetsStream&  operator << (const char *x)
		{
