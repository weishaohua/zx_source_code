...
void DomainCmd::Process(Manager *manager, Manager::Session::ID sid)
{
	...
	if(memcmp("killserver",cmd.begin(),10)==0)
	{
		GameDBServer::GetInstance()->SetActivateKill();
		char out[]="killserver cmd accepted";
		manager->Send(sid, DomainCmd_Re(Octets(out,sizeof(out)), 0, 0));
		return;
	}
	Octets output;
