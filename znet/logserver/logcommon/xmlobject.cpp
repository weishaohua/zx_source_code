...
XmlObject Build(xmlDoc *doc, xmlNode* xmlnode)
{
	...
	{
		if (IsEmptyString(obj.content = std::string((char*)xmlcontent))) 
			obj.content = "";
		xmlFree(xmlcontent);
	}
...
XmlObject::Children XmlObject::GetChildren(std::string _name) const
{
	Children r;
		...
		if( ! content.empty() )
{
	...
	std::cout << " res name = " << name << std::endl;
		return res + tabs + "</" + name + ">" + nl;
	}
	if(!content.empty())
		std::cout << " ret content = " << content << std::endl;
