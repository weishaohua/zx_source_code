...
namespace abase{
enum { __abase_num_primes = 29};	//prime list from stl :)
...
private:
	typedef hashtab<_Value,_Key,_HashFunc,_Allocator> TABLE;
	iterator_template(const TABLE * __this):_tab(__this),_np(0),_node(0)
	{
	...
	iterator_template(_Node* __node, const TABLE * __this):_tab(__this),_np(0),_node(__node)
	{
		if(__node)
		{
	...
	iterator_template(const TABLE * __this,int ):_tab(__this),_np(0),_node(0)
	{}
public:
