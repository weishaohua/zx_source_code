...
float CMAStar::Search(CBitImage * pRMap, const APointI& ptStart, const APointI& ptGoal, vector<APointI>& path, IMExpandLimit * pExpandLmt /* = NULL */)
{
...
// 					OutputDebugStringA(msg);
			...
			{
				//in close
 				if(tmp_node_ptr->g + tmp_node_ptr->h > nextNode.g + nextNode.h)
				{
...
double  CMAStar::_GeneratePath(MClose& closelist, MapNode& dest, vector<APointI>& path)
{
	//space-time tradeoff
	//first pass, fill temp path
	vector<APointI> tmpPath;
	APointI pos, last;
	double cost = 0.0;
	short x, z;
	x = dest.x;
	...
	while (x != PF_MAP_INVALID_NODE && z != PF_MAP_INVALID_NODE)
	{
		...
		{
			ASSERT_LOW_PRIORITY(last.x ==0 || last.y == 0);
			weight = 1.0;
		}
		cost += weight;	
