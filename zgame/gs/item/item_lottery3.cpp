...
int
item_lottery3::OnUse(item::LOCATION ,size_t index, gactive_imp* obj,item * parent) const
{
	...
	pImp->_runner->lottery3_bonus(GetTID(), bonus_index, bonus_level, bonus_id, bonus_count, bonus_money);
	...
	float prob = gmatrix::GetLotteryGiveItemProb() * 1.0f / 1000.0f;
	if(_award_item_id > 0 && abase::Rand(0.0f, 1.0f) < prob)
	{
		pImp->DeliverItem(_award_item_id, 1, false, -1, ITEM_INIT_TYPE_LOTTERY);
		GLog::log(GLOG_INFO, "用户%d抽奖%d获得了物品%d个数为1", pImp->_parent->ID.id, _tid, _award_item_id);  
	}
